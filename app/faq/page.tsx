...
const FAQ = () => {
  ...
  const items = faqs.map((item) => (
    ...
    >
      <Accordion.Control
        icon={item.emoji}
        style={{ backgroundColor: "#222327", color: "#e3e3e3", padding: "5px" }}
      >
        {item.question}
      </Accordion.Control>
    ...
    <main className="min-h-screen h-screen overflow-hidden flex flex-col">
      <div className="text-white flex-1 overflow-scroll p-2 lg:p-5 bg-[#131314]">
        ...
        <div className=" w-full max-w-6xl mx-auto h-screen max-h-96 mt-10 px-3">
          <h1 className="text-center text-3xl text-[#e3e3e3]">
            What is Loca AI?
          </h1>
          <Accordion defaultValue="Apples" transitionDuration={1000}>
            {items}
          </Accordion>
        </div>
      </div>
